...
impl TaskManager {
    ...
    fn make_a_syscall(&self, syscall_id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
    ...
    fn get_task_calltimes(&self) -> [u32; MAX_SYSCALL_NUM] {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].syscall_times.clone()
    }
    
    fn get_task_rtime(&self) -> usize {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
...
// update syscall_times
pub fn make_a_syscall(syscall_id: usize) {
    TASK_MANAGER.make_a_syscall(syscall_id);
}
...
pub fn get_task_calltimes() -> [u32; MAX_SYSCALL_NUM] {
    TASK_MANAGER.get_task_calltimes()
}

pub fn get_task_rtime() -> usize {
    TASK_MANAGER.get_task_rtime()
}
